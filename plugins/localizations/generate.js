...
/** Info.plist values that are read by the system rather than by our code. */
...
function writeIosStrings(catalogue, platformRoot, appName) {
  const targets = [
  ...
  for (const language of Object.keys(catalogue)) {
    for (const target of targets) {
      ...
      const infoPlist = IOS_INFO_PLIST_STRINGS.map(
        (entry) => `"${entry.entry}" = "${escapeStrings(value(catalogue, language, entry.key))}";`,
      ).join('\n');
      fs.writeFileSync(
